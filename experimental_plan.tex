...
\rowstyle{\bfseries}
  ...
  E2 & RQ1 & Norwegian Roads Dataset Vbase & Bootstrapping loss performance at different levels of label noise\\
...
Most of the experiments listed in \ref{tab:planned_experiments} compare results from a certain method with a baseline. The experimental runs results in observable samples from both these distributions. By statistical hypothesis testing and the Welch's t test, it is possible to assert whether it is plausible that the samples were produced from distinct distributions \todo{Weird}. The distributions are described by the mean $\mu_i$ and variance $\sigma_i$. The null hypothesis $H_0\colon \mu_1 = \mu_2$ is rejected in favour of the alternate hypothesis $H_1\colon\mu_1 \neq \mu_2$ , based on the p-value found by the t-test.  If the p-value is below the significance level $\alpha$ of 0.1, $H_0$ is rejected. This indicate that it is unlikely that the samples have been produced from the same underlying distribution.\\

The Welch's t test is derived from the Student t test, which is suitable in situations where the variance and mean of the underlying distributions are unknown \citep{walpole_probability}. These are estimated from the samples. Unlike the Student t test, the Welch's t test does not assume that the variance of the distributions are equal. However for small sample sizes \todo{True?}, the tests assume that the underlying distributions are normal. This seemed to be true for the experiments in this chapter, based on visual interpretation of normal Q-Q plots created from the measurements. Unfortunately, whether or not the underlying distributions were normal could not be confidently asserted because of the small sample size. In appendix -\todo{Create plot appendix}, examples of these plots can be found. \\ 

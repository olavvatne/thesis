...
In summary, the composition of the first stage has a big effect on the outcome. The results demonstrate that the proposed curriculum strategy, which is based on estimating inconsistencies between labels and predictions, works well in practice.

\subsection{Performance of the Road Detection System}
\todo[inline]{Still testing so analysis kind of hard still}
...
\todo[inline]{Ripe for mentioning the CRF and post processing neural network}
...
\end{subfigure}
\caption[E6 - Qualitiative results of the road extraction system ]{E6 - Example of model's road detection performance. The aerial image is part of the test set in Massachusetts Roads Dataset} \label{fig:E6_performance}
\end{figure}

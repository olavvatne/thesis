...
A typical convolutional layer in a network consists of three stages. First, convolution creates all pre-synaptic activations. Second, an activation function is applied to the pre-synaptic activation for all units in the layer. The rectified linear unit is a popular choice, and outputs either 0 or the pre-synaptic activation, depending on which is biggest. $f(x) = max(0, a)$. Finally the pooling function modifies the output of the layer. 

...
A typical convolutional layer in a network consists of three stages. First, convolution sums the weighted inputs. Second, an activation function is applied to the summed and weighted inputs \todo{Sounds weird} for all units in the layer. The rectified linear unit is a popular choice, and outputs either 0 or the weighted sum, depending on which is biggest. $f(x) = max(0, a)$. Finally the pooling function modifies the output of the layer. 

...
\todo[inline]{IN TEMPLATE CALLED MOTIVATION: Application driven or methodology driven. Research focus. Other research. Why goals and questions are important to address. Present literate review. (overview of motivating elements}
...
They formulated extraction of road pixels from aerial images as a patch-based semantic segmentation problem. 
The goal of the model is to predict whether or not pixels belong to a road class, given an image patch. This is achieved by having the neural network learn $p(N(M(i,j), w_m) \mid N(S(i,j), w_s)$, where $S$ is an aerial image and $M$ is a corresponding road label image. $M(i,j) = 1$ if $S(i,j)$ is a road pixel and 0 otherwise.  $N(I(i,j), w)$  denotes a $w \times w$ large patch of pixels centered at location (i,j). \\
...
\cite{Mokhtarzade_road_ann} proposed using a shallow neural network for road detection. For this approach, a normalized $3 \times 3$ neighbourhood of pixel values was used as input features to a neural network. \\
...
%Setting the weights of this layer to the identity, $q_{ji} = 1$, where j = i, is equivalent to not having a noise layer. \\
...
$$L(q,y) = - \sum\limits_{k=1}^L [\beta y_k + (1-\beta)z_k]log(q_k),$$

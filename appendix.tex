...
In this appendix results from the best performing convolutional neural networks are displayed. The precision and recall breakeven point of the model trained on the Massachusetts Roads Dataset is --\todo{Breakeven values}. Whereas, the breakeven point for the best model trained on the Norwegian Roads Dataset is --.  
...
\item Background discussion - Write a summary of related works, and short emphasis on elements important for the rest of the thesis. Extraction systems, label noise, curriculum learning.
...
\item Background theory - Road extraction system. Describe problem, illustrate by pictures. Keep it entirely general! Explain terminology, such as labels and map data etc etc. Move the introduction from related works for example.
\item Introduce moments in contributions. Halv-setninger.
\item Footnote for mnih and saito - average of 10 or 1?
\item Make CRF description less confusing
\item Make curriculum math less confusing
\item Future work does not need to contain solutions, only proposals. K-means , nope that out.
\end{itemize}
 
\todo{field of research, brief motivation for the work, what the research topic is, the research approach(es) applied. contributions}
...
Chapter 5.
\todo{Limits and merits. Propertly in discussion summary of results?}

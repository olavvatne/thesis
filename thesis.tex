...
\section*{Abstract}
This thesis presents a brief introduction to aerial road detection, and semantic segmentation of images. Aerial imagery is often automatically generated from existing map data, which causes the dataset to be afflicted by label noise. Supervised training with datasets having inconsistent labelling will penalize the classifier for making correct predictions, and can impact the resulting performance of the classifier. The thesis investigates different approaches to decrease the impact of noisy labels in deep neural networks. This includes modifying the loss function by the bootstrapping method, and adjusting the training regime through the use of curriculum learning.\\
